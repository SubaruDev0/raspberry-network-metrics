...
# -*- coding: utf-8 -*-
"""
Generador de Gráficos - Cliente TCP
Genera visualizaciones de las métricas capturadas por el cliente TCP
"""
...
REGISTROS_DIR = os.path.join(PROJECT_ROOT, 'registros', 'tcp')
GRAFICOS_DIR = os.path.join(PROJECT_ROOT, 'graficos', 'tcp', 'cliente')
...
def cargar_datos_cliente():
    """Carga datos del cliente TCP"""
    try:
        cliente_csv = os.path.join(REGISTROS_DIR, 'metricas_cliente.csv')
        df_cliente = pd.read_csv(cliente_csv)

        print(f"[INFO] Datos del cliente TCP cargados: {len(df_cliente)} registros")
...
def generar_graficos_cliente(df_cliente):
    """Genera gráficos específicos del cliente TCP"""
    print("\n--- GENERANDO GRÁFICOS DEL CLIENTE TCP ---")
    ...
    plt.title('RTT vs Tiempo - Cliente TCP', fontsize=14, fontweight='bold')
    ...
    plt.title('Jitter vs Tiempo - Cliente TCP', fontsize=14, fontweight='bold')
    ...
    plt.title('Throughput de Recepción vs Tiempo - Cliente TCP', fontsize=14, fontweight='bold')
...
def main():
    """Función principal"""
    print("Generador de Gráficos - Cliente TCP")
    print("=" * 40)

    df_cliente = cargar_datos_cliente()
    if df_cliente is None:
        print("[ERROR] No hay datos del cliente TCP para generar gráficos")
        return

    generar_graficos_cliente(df_cliente)
    print(f"\n✅ Gráficos del cliente TCP guardados en: {GRAFICOS_DIR}")

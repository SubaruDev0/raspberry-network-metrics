...
# -*- coding: utf-8 -*-
"""
Generador de Gráficos - Servidor UDP
Genera visualizaciones de las métricas capturadas por el servidor UDP
"""
import os
import pandas as pd
...
GRAFICOS_DIR = os.path.join(PROJECT_ROOT, 'graficos', 'udp', 'server')
...
def cargar_datos_servidor():
    """Carga datos del servidor UDP"""
    try:
        servidor_csv = os.path.join(REGISTROS_DIR, 'metricas_servidor.csv')
        df_servidor = pd.read_csv(servidor_csv)
        print(f"[INFO] Datos del servidor UDP cargados: {len(df_servidor)} registros")
        ...
        print(f"[ERROR] No se pudieron cargar los datos del servidor UDP: {e}")
        return None
def generar_graficos_servidor(df_servidor):
    """Genera gráficos específicos del servidor UDP"""
    print("\n--- GENERANDO GRÁFICOS DEL SERVIDOR UDP ---")
    # 1. RTT vs Tiempo
    ...
    plt.title('RTT vs Tiempo - Servidor UDP', fontsize=14, fontweight='bold')
    ...
    plt.title('Tamaño de Payload vs RTT - Servidor UDP', fontsize=14, fontweight='bold')
    ...
    plt.title('Jitter vs Tiempo - Servidor UDP', fontsize=14, fontweight='bold')
...
def main():
    """Función principal"""
    print("Generador de Gráficos - Servidor UDP")
    print("=" * 40)
    df_servidor = cargar_datos_servidor()
    if df_servidor is None:
        print("[ERROR] No hay datos del servidor UDP para generar gráficos")
        return
    generar_graficos_servidor(df_servidor)
    print(f"\n✅ Gráficos del servidor UDP guardados en: {GRAFICOS_DIR}")

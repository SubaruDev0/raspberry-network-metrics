...
# -*- coding: utf-8 -*-
"""
Generador de Gráficos - Servidor TCP
Genera visualizaciones de las métricas capturadas por el servidor TCP
"""
...
GRAFICOS_DIR = os.path.join(PROJECT_ROOT, 'graficos', 'tcp', 'server')
...
def cargar_datos_servidor():
    """Carga datos del servidor TCP"""
    try:
        servidor_csv = os.path.join(REGISTROS_DIR, 'metricas_servidor.csv')
        df_servidor = pd.read_csv(servidor_csv)

        print(f"[INFO] Datos del servidor TCP cargados: {len(df_servidor)} registros")
...
def generar_graficos_servidor(df_servidor):
    """Genera gráficos específicos del servidor TCP"""
    print("\n--- GENERANDO GRÁFICOS DEL SERVIDOR TCP ---")
...
def main():
    """Función principal"""
    print("Generador de Gráficos - Servidor TCP")
    print("=" * 40)

    df_servidor = cargar_datos_servidor()
    if df_servidor is None:
        print("[ERROR] No hay datos del servidor TCP para generar gráficos")
        return

    generar_graficos_servidor(df_servidor)
    print(f"\n✅ Gráficos del servidor TCP guardados en: {GRAFICOS_DIR}")

...
GRAFICOS_DIR = os.path.join(PROJECT_ROOT, 'graficos', 'cliente')
...
def cargar_datos_cliente():
    ...
    try:
        ...
        print(f"[INFO] Datos del cliente cargados: {len(df_cliente)} registros")
...
def generar_graficos_cliente(df_cliente):
    """Genera gráficos específicos del cliente"""
    print("\n--- GENERANDO GRÁFICOS DEL CLIENTE ---")
...
def main():
    """Función principal"""
    print("Generador de Gráficos - Cliente")
    print("=" * 40)

    df_cliente = cargar_datos_cliente()
    if df_cliente is None:
        print("[ERROR] No hay datos del cliente para generar gráficos")
        return

    generar_graficos_cliente(df_cliente)
    print(f"\n✅ Gráficos del cliente guardados en: {GRAFICOS_DIR}")
